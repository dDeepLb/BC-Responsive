...
import { sendNewVersion } from "./Versions";
import { OnLogin } from "./Login";

export function LoadHooks() {
  ResponsiveMod.hookFunction("ChatRoomMessage", HOOK_PRIORITY.OVERRIDE_BEHAVIOR, (args, next) => {
    next(args);
    chatMessageHandler.Run(Player, args[0] as IChatRoomMessage);
  });

  ResponsiveMod.hookFunction("ActivityOrgasmStart", HOOK_PRIORITY.OVERRIDE_BEHAVIOR, (args, next) => {
    isOrgasm = true;
    OrgasmHandle(args[0] as Character);
    next(args);
  });

  ResponsiveMod.hookFunction("LoginResponse", HOOK_PRIORITY.ADD_BEHAVIOR, (args, next) => {
    next(args);
    OnLogin();
  });

  ResponsiveMod.hookFunction("ChatRoomSync", HOOK_PRIORITY.OBSERVE, (args, next) => {
    next(args);
    sendNewVersion();
  });
  ResponsiveMod.hookFunction("ChatRoomLeave", HOOK_PRIORITY.ADD_BEHAVIOR, (args, next) => {
    LeaveMessage();
    next(args);
  });

  ResponsiveMod.hookFunction("ServerAccountBeep", HOOK_PRIORITY.ADD_BEHAVIOR, (args, next) => {
    let data = args[0];
