...
function BaseMoan(Arousal: number, shift?: number) {
	...
	const Tkeys: (keyof ResponsiveSetting)[] = [
		"low",
		"low",
		"light",
		"medium",
...
function MixMoan(player: Character, t: MoanType, act: string) {
	if (player && player.ArousalSettings) {
		let actFactor = player.ArousalSettings.Activity.find(
			(_) => _.Name === act
		)?.Self;
		if (!actFactor) return "";
...
export function MasturbateMoan(
	player: Character,
	masturSrc:
		| "MasturbateHand"
		| "MasturbateFist"
	...
	ChatRoomAutoInterceptMessage(
		ElementValue("InputChat"),
		BaseMoanStepped(player, masturSrc)
	);
}

export function PainMessage(
	player: Character,
	painSrc:
		| "Bite"
		| "Slap"
	...
	if (
		activityInfo.ActivityName === "LSCG_SharkBite" &&
		activityInfo.ActivityGroup === "ItemNose"
	)
		return BoopMessage(player, "LSCG_SharkBite", activityInfo);
	ChatRoomAutoInterceptMessage(
		ElementValue("InputChat"),
		MixMoan(player, MoanType.Pain, painSrc)
	);
}
...
export function OrgasmMessage(player: Character) {
	...
	ChatRoomAutoInterceptMessage(
		ElementValue("InputChat"),
		TypedMoan(MoanType.Orgasm)
	);
}

export function TickleMessage(
	player: Character,
	tickleSrc: "TickleItem" | "Tickle",
	activityInfo: ActivityInfo
) {
	if (
		!DataManager.instance.data.tickle ||
		activityInfo.SourceCharacter.MemberNumber ===
			activityInfo.TargetCharacter.MemberNumber
	)
		return;
	ChatRoomAutoInterceptMessage(
		ElementValue("InputChat"),
		MixMoan(player, MoanType.Tickle, tickleSrc)
	);
}
export function BoopMessage(
	player: Character,
	boopSrc: "Pet" | "LSCG_SharkBite",
	activityInfo: ActivityInfo
) {
	if (
		!DataManager.instance.data.boop ||
		activityInfo.ActivityGroup === "ItemHead"
	)
		return;
	ChatRoomAutoInterceptMessage(
		ElementValue("InputChat"),
		TypedMoan(MoanType.Boop)
	);
}
